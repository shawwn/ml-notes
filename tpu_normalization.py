...
class BatchNormalization(tf.layers.BatchNormalization):
  ...
  def add_weight(self, name, **kws):
    print('add_weight', name, kws)
    experimental_autocast = kws.pop('experimental_autocast', None)
    return tft.globalvar(name, **kws)


class CrossReplicaBatchNormalization(BatchNormalization):
  def __init__(self, num_distributed_groups=1, **kwargs):
    super(CrossReplicaBatchNormalization, self).__init__(
...
def cross_replica_batch_normalization(inputs,
  ...
  #         cross_replica_average, num_groups=num_distributed_groups),
  #     **kwargs)
  layer = CrossReplicaBatchNormalization(num_distributed_groups=num_distributed_groups, **kwargs)
