...
class Module(object):
    ...
    def globalvar(self, name, **kws):
      return tft.globalvar(name, **kws)
    
    def localvar(self, name, **kws):
      return tft.localvar(name, **kws)
...
def zeros_(tensor):
  return tensor.assign(tf.zeros(tensor.shape, dtype=tensor.dtype))


def ones_(tensor):
  return tensor.assign(tf.ones(tensor.shape, dtype=tensor.dtype))
...
class _NormBase(Module):
    ...
    def reset_running_stats(self) -> None:
        if self.track_running_stats:
            ...
            # self.accumulated_mean = self.accumulated_mean.assign(self.accumulated_mean.initial_value)
            # self.accumulated_var = self.accumulated_var.assign(self.accumulated_var.initial_value)
            # self.accumulation_counter = self.accumulation_counter.assign(self.accumulation_counter.initial_value)
            self.accumulated_mean = zeros_(self.accumulated_mean)
            self.accumulated_var = ones_(self.accumulated_var)
            self.accumulation_counter = zeros_(self.accumulation_counter)
