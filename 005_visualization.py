...
g_vars = [x for x in tf.all_variables() if 'Discriminator' not in x.name]
d_vars = [x for x in tf.all_variables() if 'Discriminator' in x.name]
ckpt = 'gs://tpu-usc1/models/biggan-deep-512/variables' if deep else 'gs://tpu-usc1/models/biggan-256/variables'
...
def datdisk(data, filename='bgd512_0.png' if deep else 'bg256_0.png'):
...
def biginterp(filename='bginterp_%03d.png', n=60, truncation=0.5, seed_a=random.randint(0, 1000), seed_b=random.randint(0, 1000), class_id=random.randint(0, 1000), class_id_b=None):
...
update = tf.io.write_file('gs://tpu-usc1/carousel/biggan-256.png', pngs)

while True: r(update, {z: truncated_z_sample(1, 140, truncation=0.5, seed=random.randint(0, 1000)), y_index: [random.randint(0, 1000)]}); time.sleep(3.0);
